...
__global__ void kernel(Sphere *s_begin, Sphere *s_end, unsigned char *ptr, float p) {
...
void render(uchar4* devPtr, void *datablock, int tick, float state) {
    ...
    dim3    threads(16,16);
    kernel<<<grids, threads>>>(s_begin, s_end, (unsigned char *)devPtr, state);
...
int main() {
    GPUAnimBitmap bitmap(DIM, DIM, "Ray (without const memory)");
    ...
    cudaMalloc(&s_begin, sizeof(Sphere) * SPHERES);
    cudaMalloc(&s_end, sizeof(Sphere) * SPHERES);
    HANDLE_ERROR( cudaMemcpy( s_begin, temp_s, sizeof(Sphere) * SPHERES, cudaMemcpyHostToDevice ) );
    HANDLE_ERROR( cudaMemcpy( s_end, temp_s_end, sizeof(Sphere) * SPHERES, cudaMemcpyHostToDevice ) );
    free( temp_s );
    free( temp_s_end );
